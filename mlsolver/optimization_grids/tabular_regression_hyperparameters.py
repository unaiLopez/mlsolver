...
class TabularRegressionHyperparameters:
    ...
    def _load_random_forest_hyperparameters(self):
        return {
            ...
            'preprocessor__numerical__cleaner__strategy': self.numerical_clean_strategies,
            'preprocessor__categorical__cleaner__strategy': self.categorical_clean_strategies,
            'preprocessor__categorical__encoder': [OneHotEncoder(sparse=False)],
            'feature_selector__k': ['all', 3, 6, 12],
            'estimator': [RandomForestRegressor()],
            'estimator__n_estimators': np.arange(100, 800, 10),
    ...
    def _load_knn_hyperparameters(self):
        return {
            'preprocessor__numerical__scaler': self.scalers,
            'preprocessor__numerical__cleaner__strategy': self.numerical_clean_strategies,
            'preprocessor__categorical__cleaner__strategy': self.categorical_clean_strategies,
            'preprocessor__categorical__encoder': [OneHotEncoder(sparse=False)],
            'feature_selector__k': ['all', 3, 6, 12],
            ...
            'estimator__leaf_size': np.arange(10, 50, 10),
            'estimator__metric': ['euclidean', 'manhattan', 'minkowski'],
            'estimator__n_neighbors': np.arange(1, 100, 8)
        }
    ...
    def _load_svm_hyperparameters(self):
        return {
            ...
            'estimator__C': np.arange(0.1, 1, 5)
        }
