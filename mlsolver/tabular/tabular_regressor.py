...
class TabularRegressor(AutoTabular):
    ...
    def _create_feature_engineering_pipeline(self, X):
        ...
        feature_engineering_preprocessor = ColumnTransformer([
            ...
            ('categorical', categorical_pipeline, make_column_selector(dtype_include=['object','category','bool'])),
        ])
    ...
    def fit(self, X, y):
        ...
        optimization_grid.append({
            'preprocessor__numerical__scaler': [None, StandardScaler(), MinMaxScaler(), RobustScaler()],
            'preprocessor__numerical__cleaner__strategy': ['mean', 'median'],
            'preprocessor__categorical__encoder': [OneHotEncoder(sparse=False)],
            'feature_selector__k': ['all', 3, 6, 12],
            'estimator': [KNeighborsRegressor()],
            'estimator__n_neighbors': [5, 10, 15, 20, 25, 30, 50, 100]
        })

        data = pd.concat([X, y], axis=1)

        cv = create_stratified_kfolds_for_regression(data=data, target_column=data.columns[-1]) #THIS WILL CRASH IF THE PROBLEM IS A MULTIPLE COLUMN REGRESSION
